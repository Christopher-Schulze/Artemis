# TASK 047: Performance benchmark vs Lightpanda (best effort)

## Why

Quantify where Artemis stands against Lightpanda's published demo numbers.
...
## Comparison to Lightpanda

Lightpanda's published demo: **100 pages in ~5000ms = ~50ms/page on AWS m5.large**, running against real internet URLs.

The two numbers are not directly comparable:
- Lightpanda's 50ms/page includes real network RTT to the upstream sites (~30-50ms typical on AWS).
- Our 0.85ms/page excludes network RTT (httptest is in-process).
- Lightpanda runs more JS (full external scripts loaded from CDN); our pages have only inline scripts.

Apples-to-apples comparison would require:
1. Running Lightpanda binary against the same httptest fixtures to isolate engine cost
2. Running Artemis against the same real-world URL set with realistic network

Both are non-trivial harness work. **The headline conclusion** from the numbers we have: Artemis engine cost (parse + V8 + DOM + Markdown) is in the sub-millisecond range on a modern Apple Silicon host, comfortably below the 50ms Lightpanda budget that's dominated by network. The remaining performance work (TASK 042 V8 snapshot, TASK 043 HTTP/2 pool, TASK 044 zero-copy) targets cross-process competitiveness rather than overcoming a fundamental gap.
...
A side-by-side wall-clock harness against the Lightpanda binary across 933 real URLs (their demo dataset) is a separate sub-task tracked under TASK 047b when we have appetite to set up the demo repo and run both processes head-to-head. Today's data is enough to assert "engine cost is in the right zone".
