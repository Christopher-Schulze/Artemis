...
import (
	...
	"github.com/Christopher-Schulze/Artemis/third_party/utls/dicttls"
...
func (e *SupportedCurvesExtension) UnmarshalJSON(data []byte) error {
	...
	for _, namedGroup := range namedGroups.NamedGroupList {
		if namedGroup == "GREASE" {
			e.Curves = append(e.Curves, GREASE_RESERVED)
			continue
		}
...
func (e *SignatureAlgorithmsExtension) UnmarshalJSON(data []byte) error {
	...
	for _, sigScheme := range signatureAlgorithms.Algorithms {
		if sigScheme == "GREASE" {
			e.SupportedSignatureAlgorithms = append(e.SupportedSignatureAlgorithms, GREASE_RESERVED)
			continue
		}
...
// upstream-note: update when this extension is implemented in crypto/tls
...
func (e *PSKKeyExchangeModesExtension) Write(b []byte) (int, error) {
	...
	// upstream-note: PSK Modes have their own form of GREASE-ing which is not currently implemented
	// the current functionality will NOT re-GREASE/re-randomize these values when using a fingerprinted spec
	// https://github.com/Christopher-Schulze/Artemis/third_party/utls/pull/58#discussion_r522354105
...
func (e *SupportedVersionsExtension) UnmarshalJSON(b []byte) error {
	...
	for _, version := range supportedVersions.Versions {
		switch version {
		case "GREASE":
			e.Versions = append(e.Versions, GREASE_RESERVED)
		case "TLS 1.3":
...
func (e *RenegotiationInfoExtension) writeToUConn(uc *UConn) error {
	...
	case RenegotiateFreelyAsClient:
		...
		// upstream-note: don't do backward propagation here
		if uc.handshakes > 0 {
