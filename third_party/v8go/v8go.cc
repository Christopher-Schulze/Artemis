...
// m_snapshotCreator wraps v8::SnapshotCreator. Lifecycle: New -> RunScript*
// -> CreateBlob -> Delete. CreateBlob serialises the default context and
// the dtor of SnapshotCreator disposes the wrapped Isolate. The produced
// blob bytes are heap-copied so they outlive the SnapshotCreator.
struct m_snapshotCreator {
  SnapshotCreator* creator;
  Isolate* iso;             // borrowed from creator; do not dispose manually
...
IsolatePtr NewIsolate() {
  ...
  ctx->iso = iso;
  iso->SetData(0, ctx);
  iso->SetData(1, nullptr);  // no startup data attached

  return iso;
}

// NewIsolateWithSnapshot creates an Isolate seeded with a previously
// produced startup snapshot. New Contexts on this Isolate deserialise
// the snapshot's default context, skipping the parse + first-run cost
// for whatever JS was baked into the snapshot.
//
// V8 requires the StartupData buffer to outlive the Isolate. We heap-
// allocate a copy and stash the StartupData* in slot 1 so IsolateDispose
// can free it.
IsolatePtr NewIsolateWithSnapshot(const uint8_t* data, int len) {
  ...
  StartupData* snap = new StartupData;
  snap->data = reinterpret_cast<const char*>(data_copy);
  snap->raw_size = len;

  Isolate::CreateParams params;
  params.snapshot_blob = snap;
  params.array_buffer_allocator = default_allocator;
  Isolate* iso = Isolate::New(params);
  Locker locker(iso);
  Isolate::Scope isolate_scope(iso);
...
m_snapshotCreator* SnapshotCreatorNew() {
  m_snapshotCreator* sc = new m_snapshotCreator;
  // external_references = nullptr: we only snapshot pure JS, no native
  // callbacks are baked in. Disambiguate the SnapshotCreator overload by
  // providing both args explicitly (existing_blob = nullptr).
  sc->creator = new SnapshotCreator(static_cast<const intptr_t*>(nullptr),
                                    static_cast<StartupData*>(nullptr));
  sc->iso = sc->creator->GetIsolate();
  sc->finalized = false;

  Isolate::Scope isolate_scope(sc->iso);
  HandleScope handle_scope(sc->iso);
  Local<Context> ctx = Context::New(sc->iso);
  sc->ctx.Reset(sc->iso, ctx);
  return sc;
}

RtnError SnapshotCreatorRunScript(m_snapshotCreator* sc,
                                  const char* source,
                                  const char* origin) {
  RtnError err = {nullptr, nullptr, nullptr};
  if (sc == nullptr || sc->finalized) {
    err.msg = CopyString("snapshot creator unavailable");
    return err;
  }
  Isolate::Scope isolate_scope(sc->iso);
  HandleScope handle_scope(sc->iso);
  Local<Context> ctx = Local<Context>::New(sc->iso, sc->ctx);
  Context::Scope context_scope(ctx);

  Local<String> src;
  if (!String::NewFromUtf8(sc->iso, source).ToLocal(&src)) {
    err.msg = CopyString("invalid source string");
    return err;
  }
  Local<String> ori;
  if (!String::NewFromUtf8(sc->iso, origin).ToLocal(&ori)) {
    err.msg = CopyString("invalid origin string");
    return err;
  }
  ScriptOrigin so(sc->iso, ori);
  TryCatch try_catch(sc->iso);
  Local<Script> script;
  if (!Script::Compile(ctx, src, &so).ToLocal(&script)) {
    return ExceptionError(try_catch, sc->iso, ctx);
  }
  Local<Value> result;
...
SnapshotBlob SnapshotCreatorCreateBlob(m_snapshotCreator* sc) {
  SnapshotBlob result = {nullptr, 0, {nullptr, nullptr, nullptr}};
  if (sc == nullptr || sc->finalized) {
    result.error.msg = CopyString("snapshot creator unavailable");
    return result;
  }
  {
    Isolate::Scope isolate_scope(sc->iso);
    HandleScope handle_scope(sc->iso);
    Local<Context> ctx = Local<Context>::New(sc->iso, sc->ctx);
    sc->creator->SetDefaultContext(ctx);
    sc->ctx.Reset();
  }
  StartupData blob =
      sc->creator->CreateBlob(SnapshotCreator::FunctionCodeHandling::kClear);
  if (blob.data == nullptr || blob.raw_size <= 0) {
    result.error.msg = CopyString("CreateBlob returned empty snapshot");
    return result;
  }
  uint8_t* copy = (uint8_t*)malloc(blob.raw_size);
  memcpy(copy, blob.data, blob.raw_size);
  delete[] blob.data;
  result.data = copy;
  result.length = blob.raw_size;
  sc->finalized = true;
  return result;
}

void SnapshotCreatorDelete(m_snapshotCreator* sc) {
  if (sc == nullptr) {
    return;
  }
  // ~SnapshotCreator disposes the wrapped Isolate. The Persistent<Context>
  // must be reset before that to avoid dangling.
  if (!sc->ctx.IsEmpty()) {
    Isolate::Scope isolate_scope(sc->iso);
    HandleScope handle_scope(sc->iso);
    sc->ctx.Reset();
...
void IsolateDispose(IsolatePtr iso) {
  ...
  // If this isolate was created from a startup snapshot, retrieve the
  // StartupData* we stashed in slot 1 BEFORE Dispose invalidates the
  // isolate. After Dispose, free the heap-allocated bytes.
  StartupData* snap = static_cast<StartupData*>(iso->GetData(1));
  iso->Dispose();
  if (snap != nullptr) {
    delete[] snap->data;
    delete snap;
...
// ObjectSetMany sets `count` (key, value) pairs on `ptr` in a single
// cgo crossing. Artemis uses this on hot install paths (buildLocation,
// setNavigatorFields) where many string properties are set in sequence;
// each individual ObjectSet otherwise costs one cgo call.
void ObjectSetMany(ValuePtr ptr, int count,
                   const char** keys,
                   ValuePtr* vals) {
  LOCAL_OBJECT(ptr);
  for (int i = 0; i < count; i++) {
    Local<String> k =
        String::NewFromUtf8(iso, keys[i], NewStringType::kNormal)
            .ToLocalChecked();
    m_value* v = static_cast<m_value*>(vals[i]);
    obj->Set(local_ctx, k, v->ptr.Get(iso)).Check();
  }
}
