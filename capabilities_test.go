...
func TestCapabilityRegistryReturnsDefensiveCopy(t *testing.T) {
	...
	if len(capabilities[2].ArtemisTools) == 0 {
		t.Fatal("test requires a capability with an Artemis tool")
	}
	capabilities[2].ArtemisTools[0] = "mutated"

	fresh := Capabilities()
	if fresh[0].ID == "mutated" || fresh[2].ArtemisTools[0] == "mutated" {
...
func TestUnavailableCapabilities(t *testing.T) {
	for _, capability := range Capabilities() {
		if capability.State == SupportUnavailable && len(capability.ArtemisTools) != 0 {
			t.Fatalf("unavailable capability %q exposes tools %v", capability.ID, capability.ArtemisTools)
		}
	}
